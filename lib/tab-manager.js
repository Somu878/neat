import { getSettings, removeTabMeta, getTabMeta } from "./storage.js";
...
export async function findStaleTabs(tabs) {
  ...
  return tabs.filter((tab) => {
    ...
    const lastAccessed = tab.lastAccessed || meta[tab.id]?.lastAccessed;
    if (!lastAccessed) return false;
    return now - lastAccessed > threshold;
  });
}
...
export async function findSleepCandidates(tabs) {
  const settings = await getSettings();
  if (!settings.enableSleep) return [];
  const meta = await getTabMeta();
  const threshold = (settings.sleepAfterMinutes || 60) * 60 * 1000;
  const now = Date.now();
  return tabs.filter((tab) => {
    if (tab.discarded || tab.active || tab.pinned) return false;
    if (isExcluded(tab.url)) return false;
    if (isWhitelisted(tab.url, settings.whitelist)) return false;
    const lastAccessed = tab.lastAccessed || meta[tab.id]?.lastAccessed;
    if (!lastAccessed) return false;
...
export async function sleepTabs(tabs) {
  let slept = 0;
  for (const tab of tabs) {
    try {
